...
async function getPRDetails(prNumber) {
  for (let i = 0; i < 6; i++) {
    ...
    const output = runCmd(`gh pr view ${prNumber} --json mergeable,mergeStateStatus,body,title,labels`);
    if (!output) {
...
async function processPR(prNumber, prDetails = null) {
  ...
  console.log(`PR #${prNumber} stats: mergeable=${pr.mergeable}, mergeStateStatus=${pr.mergeStateStatus}, hasConflictLabel=${hasLabel}`);

  if (!isConflicting) {
    // If it's mergeable now, but has the conflict label, remove it
    if (hasLabel) {
      console.log(`PR #${prNumber} is no longer conflicting. Removing 'jules-conflict-notified' label...`);
  ...
  console.log(`PR #${prNumber} has conflicts and hasn't been notified yet. Generating conflict diff...`);

  // Ensure label exists in repo
  runCmd(`gh label create jules-conflict-notified --color f1a2b3 --description "Notified Jules of merge conflict" 2>/dev/null || true`);
...
async function main() {
  ...
  if (prNumberArg) {
    ...
    const output = runCmd(`gh pr list --json number,mergeable,mergeStateStatus,body,title,labels --limit 100`);
    if (!output) {
    ...
    try {
      ...
      const julesPRs = prs.filter(pr => pr.body && pr.body.includes("PR created automatically by Jules"));
